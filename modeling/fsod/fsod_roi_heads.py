...
class FsodRes5ROIHeads(ROIHeads):
    ...
    def __init__(self, cfg, input_shape):
        ...
        # stride=16
        # pooler_scales = (1.0 / ShapeSpec(channels=1024, height=None, width=None, stride=16).stride)
        # pooler_scales = (1.0 / 2)
        ...
        # torch.nn.init.kaiming_normal_(self.res5, a=0, mode='fan_in', nonlinearity='relu')
        # out_channels_box_predictor = 256
    ...
    def _shared_roi_transform(self, features, boxes):  # feature是带batch的tensor形式， boxes是list的 boxes
    ...
    def forward(self, images, features, support_box_features, proposals, targets=None):  # proposals [100,4]
        """
        ...
        if self.training:
            ...
            if storage.iter <= 2000:
                proposals = self.label_and_sample_proposals(proposals, targets)
            else:
                with torch.no_grad():
                    proposals_clone = proposals'''
            proposals = self.label_and_sample_proposals(proposals, targets)

        del targets
        proposal_boxes = [x.proposal_boxes for x in proposals]  # 此处的 boxes 是以类 boxes的形式存在, 因为 _shared_roi_transform的输入是此形式
        # 初始的proposals是list形式的instances，instances中fields=proposla_boxes中有相应的2000个boxes

        '''proposal_boxes = []
        for i in range(100):
            proposal = Boxes(proposals[i].unsqueeze(0))
            proposal_boxes.append(proposal)'''

        # proposal_boxes = [Boxes(proposals)]
        box_features = self._shared_roi_transform([features], proposal_boxes)

        # box_features = self.roi_pooling([features], proposal_boxes)

        '''box_features = self.roi_pooling(
    ...
    def eval_with_support(self, images, features, support_proposals_dict, support_box_features_dict):
        ...
        for cls_id, proposals in support_proposals_dict.items():
            # perm = torch.randperm(500)[:100]
            # candidate = proposals[0].proposal_boxes[perm]
            # proposals[0].remove('proposal_boxes')
            # proposals[0].set('proposal_boxes', candidate)
        ...
        for i in range(20):
            box_features = self._shared_roi_transform(
                [features], [proposal_boxes[0][i*100:(i+1)*100]]
            )
            box_feature_ls.append(box_features)
            del box_features

        box_features = torch.stack(box_feature_ls, dim=0)
        box_features_db = self._shared_roi_transform(
            [features], proposal_boxes
        )
        ...
        for cls_id in cls_ls:
            ...
            # query_features = box_features[cnt * 500:(cnt + 1) * 500]  ########
            # query_features = box_features[cnt]
            query_features = box_features_db[cnt * 100:(cnt + 1) * 100]
