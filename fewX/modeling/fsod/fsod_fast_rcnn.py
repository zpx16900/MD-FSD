...
class FsodFastRCNNOutputs(object):
    ...
    def focal_loss(self):
        """Classification loss (NLL)
                ...
                """
        self._log_accuracy()
        src_logits = self.pred_class_logits

        # fg_inds = (self.gt_classes == 0).nonzero().squeeze(-1)

        # target_classes_onehot = torch.cat(((1-self.gt_classes).unsqueeze(1), self.gt_classes.unsqueeze(1)), dim=1).type(torch.float32)
        loss_ce = self.sigmoid_focal_loss(src_logits, self.gt_classes, alpha=0.75, gamma=2)

        return loss_ce


    @staticmethod
    def sigmoid_focal_loss(inputs, targets, alpha: float = -1, gamma: float = 2):
        """
        ...
        # p_t = prob * (1 - targets) + (1 - prob) * targets  # target 筛选功能
        p_t = prob[:, 0] * (1 - targets) + prob[:, 1] * targets
    ...
    def smooth_l1_loss(self):
        ...
        # loss_box_reg = loss_box_reg / (len(fg_inds)*2)
        return loss_box_reg
...
class FsodFastRCNNOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if self.local_correlation:
            ...
            # self.norm_local1 = nn.BatchNorm2d(dim_in)
            # self.norm_local2 = nn.BatchNorm2d(dim_in)
            # self.norm_local3 = nn.BatchNorm2d(dim_in)
        ...
        if self.global_relation:
            ...
            # self.norm_global1 = nn.BatchNorm1d(dim_in)
            self.fc_2 = nn.Linear(dim_in, dim_in)
            # self.norm_global2 = nn.BatchNorm1d(dim_in)
            #self.bbox_pred_fc = nn.Linear(dim_in, 4)
            self.cls_score_fc = nn.Linear(dim_in, 2) #nn.Linear(dim_in, 2)

        self.avgpool = nn.AvgPool2d(kernel_size=3,stride=1)
        # self.avgpool1 = nn.AvgPool2d(kernel_size=8, stride=1)
        # self.avgpool2 = nn.AvgPool2d(kernel_size=7, stride=1)
        self.avgpool_fc = nn.AvgPool2d(7)
        # self.avgpool_fc = nn.AvgPool2d(16)
    ...
    '''def forward(self, x_query, x_support):
        ...
        # fc
        # x_support [1,1024,14,14] x_query [100,1024,,14,14] x_query_fc [100,1024,2,2]
        # x_support[1,256,16,16], x_query[51,256,16,16] support
        if self.global_relation:
            x_query_fc = self.avgpool_fc(x_query).squeeze(3).squeeze(2)   # 修改avgpool_fc 为16
            support_fc = self.avgpool_fc(support).squeeze(3).squeeze(2).expand_as(x_query_fc)
            cat_fc = torch.cat((x_query_fc, support_fc), 1)  # [52,512]
            out_fc_c1 = self.fc_1(cat_fc)
            out_fc_c = F.relu(self.norm_global1(out_fc_c1), inplace=True)  # [2048-->1024]linear
            out_fc_c2 = self.fc_2(out_fc_c)
            out_fc = F.relu(self.norm_global2(out_fc_c2), inplace=True)  # [1024-->1024]linear
            cls_score_fc = self.cls_score_fc(out_fc)   # [1024-->2]  linear

        # correlation
        if self.local_correlation:
            x_query_cor = self.norm_local1(self.conv_cor(x_query))
            support_cor = self.norm_local2(self.conv_cor(support))
            # x_candidate = F.conv2d(x_query_cor, support_cor.permute(1, 0, 2, 3), groups=1024)
            x_candidate = F.conv2d(x_query_cor, support_cor.permute(1, 0, 2, 3), groups=2048)
            # x_cor = F.relu(F.conv2d(x_query_cor, support_cor.permute(1, 0, 2, 3), groups=2048), inplace=True).squeeze(3).squeeze(2)
            x_cor = F.relu(self.norm_local3(x_candidate), inplace=True).squeeze(3).squeeze(2)
            cls_score_cor = self.cls_score_cor(x_cor)

        # relation
        if self.patch_relation:
            # x_query [100,1024,14,14] support [1,1024,14,14]
            support_relation = support.expand_as(x_query)  # [1,256,16,16]-->[51,256,16,16]
            x = torch.cat((x_query, support_relation), 1)  # [102,256,16,16]
            x = F.relu(self.norm_patch1(self.conv_1(x)), inplace=True)  # 5x5  # Con2d[2048,256,kernel_size(1,1),stride(1,1)]
            x = self.avgpool(x)  # pool前[100,256,14,14]
            x = F.relu(self.norm_patch2(self.conv_2(x)), inplace=True)  # 3x3  # Con2d[256,256,kernel_size(3,3),stride(1,1)]
            x = F.relu(self.norm_patch3(self.conv_3(x)), inplace=True)  # 3x3  # Con2d[256,1024,kernel_size(1,1),stride(1,1)]
            x = self.avgpool(x)  # 1x1 # pool前[100,1024,11,11]
            x = x.squeeze(3).squeeze(2)
            cls_score_pr = self.cls_score_pr(x)  # linear[1024,2]

        bbox_pred_all = self.bbox_pred_pr(x)
        # final result
        cls_score_all = cls_score_pr + cls_score_cor + cls_score_fc
        # cls_score_all = cls_score_pr + cls_score_fc
    ...
    def forward(self, x_query, x_support):
        ...
        # fc
        if self.global_relation:
            # x_query [128,2048,7,7] x_support [1,2048,7,7] x_query_fc [128,2048] support_fc [128,2048] cat_fc [128,4096]
        ...
        bbox_pred_all = self.bbox_pred_pr(
            x)  # self.bbox_pred_pr = nn.Linear(dim_in, 4) #num_bbox_reg_classes * box_dim)
        # final result
        cls_score_all = cls_score_pr + cls_score_cor + cls_score_fc
